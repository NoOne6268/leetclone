...
export default function Signup(){
    ...
    const handleClick = () => setAuthModelState((prev) => ({...prev, type: "login"}));
    ...
    return <form className='space-y-6 px-6 pb-4' onSubmit={handleRegister}>
        ...
        <div>
            ...
            <input onChange={handleChangeInput} type="displayName" name="displayName" id="displayName" className="border-2 outline-none sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5
        bg-gray-600 border-gray-500 placeholder-gray-400 text-white" placeholder="Samanvia Kadamb" />
        </div>
        <div>
            <label htmlFor="password" className="text-sm font-medium block mb-2 text-grey-300">Your Password</label>
        ...
        <button type="submit" className="w-full text-white focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center bg-brand-orange hover:bg-brand-orange-s transition duration-300 ease-in-out">
            {loading ? "Registering..." : "Register"}
        </button>
        <div className='text-sm font-medium text-gray-300'>
            Already have an account?{" "}
            <a href='#' className='text-brand-orange hover:underline' onClick={handleClick}>
                Log In
            </a>
        </div>
    </form>;
